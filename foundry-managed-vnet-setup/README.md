...
From the `foundy-managed-vnet-setup` directory, run:

```bash
cd foundy-managed-vnet-setup
azd up
```

You will be prompted to select a subscription, location, and environment name. Customize parameters in `main.parameters.json` before deploying (e.g. existing VNet resource ID, DNS zones, subnet prefixes).
