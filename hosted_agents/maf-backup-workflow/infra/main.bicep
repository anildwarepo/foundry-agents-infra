...
param aiProjectDeploymentsJson string = '[{"name":"gpt-5.4","model":{"name":"gpt-5.4","format":"OpenAI","version":"2026-03-05"},"sku":{"name":"GlobalStandard","capacity":10}}]'
...
var containerRegistryEndpoint = shouldCreateAcrForExistingProject
...
module dataDiscoveryMcp 'core/host/container-app.bicep' = {
  scope: rg
  params: {
    location: location
    tags: union(tags, {
      'azd-service-name': 'data-discovery-mcp'
    })
    containerAppName: 'ca-backup-mcp-${uniqueString(subscription().id, resourceGroupName)}'
...
output AZURE_CONTAINER_APPS_ENVIRONMENT_NAME string = dataDiscoveryMcp.outputs.managedEnvironmentName
output SERVICE_DATA_DISCOVERY_MCP_NAME string = dataDiscoveryMcp.outputs.containerAppName
output DATA_DISCOVERY_MCP_ENDPOINT string = dataDiscoveryMcp.outputs.mcpEndpoint
