...
This copy is packaged for Microsoft Foundry hosted agents. It deploys two
runtime services:
...
The original FastAPI backend and React UI remain in the repository for local
development. The Foundry entry point is `hosted_agent.py`.
...
```powershell
.\.venv\Scripts\python.exe .\data_discovery_mcp\data_discovery_mcp_server.py --port 3002
```
