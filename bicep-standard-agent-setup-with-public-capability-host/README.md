...
From the `bicep-standard-agent-setup-with-public-capability-host` directory, run:

```bash
cd bicep-standard-agent-setup-with-public-capability-host
azd up
```

You will be prompted to select a subscription, location, and environment name. To customize parameters, edit `main.parameters.json` before deploying.
